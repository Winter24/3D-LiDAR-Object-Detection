...
class VideoStreamer:
    ...
    def setup(self):
        ...
        self.model = torch.load('./scse_concate.pt', map_location=torch.device('cuda')) #cpu
        self.class_index = \
   {
        0: [(64, 128, 64),  'Bed'],
         1: [(192, 0, 64),  'Books'],
    ...
    def predict_image(self, image, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
        ...
        image = t(image)
        self.model.to(device); image=image.to(device)
        # mask = mask.to(device)
        with torch.no_grad():   
            ...
            masked = masked.cuda().squeeze(0) #cpu
        return masked
...
# Function to convert a single channel mask representation to an RGB mask.
    ...
    def generate_frames(self, image):
        ...
        #         break
            ...
            masked_frame = masked_frame.to("cpu")
            ...
            # frame_overlay = torch.tensor(frame_overlay)
            # frame_overlay = frame_overlay.to(device)
